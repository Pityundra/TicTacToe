...
import static org.openjfx.view.controller.TobbJatekosModController.gondolkozasiIdo;

public class EgyJatekosModController implements Initializable {
    @FXML
    ...
    public EgyJatekosModController() {
    }
    ...
    private void idozito(int localStartTime) {
        ...
        idozito.scheduleAtFixedRate(new TimerTask() {
            @Override
            public void run() {
                Platform.runLater(() -> {
                    if (startTime == 0) {
                        ...
                        if(jatekVegeEllenorzo("O")) {
                            startTime = -1;
                            idozito(startTime);
                            Utils.gameOverBotAlert("O");
                            wichPlayerTurn = true;
    ...
    private void gombBinding() {
        for(int i = 0; i < 5; i++) {
            for(int j = 0; j < 5; j++) {
                buttons[i][j].disableProperty().bind(labelTimer.textProperty().isEqualTo("óra")
